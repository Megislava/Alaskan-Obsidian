- CASE SENSITIVE
- can be updated/splitted with [[tmux]]
...
- `{ <command1>; <command2>; }` - block in current shell
	...
	- if directory is non-existent, create it (if successful print message), go into it, if it will not succeed print "Wasn't successful"
	```bash
	D=/tmp/foo
